...
import 'package:ai_chat_flutter/widget/custom/gradientLikeCss/web_colors.dart';
...
class CssLike {
  ...
  static Tuple2<List<Color>, List<double>> _getColorsAndStops(
    ...
    for (final param in colorStopList) {
      String colorCode = '', percentage1 = '', percentage2 = '', opacity = '1';
      ...
      if (splitParam.length > 2) {
        if (splitParam[2].contains('%')) {
          percentage2 = splitParam[2];
          if (splitParam.length > 3) {
            opacity = splitParam[3];
          }
        } else {
          opacity = splitParam[2];
        }
      }
      ...
      final color =
          _codeToColor(colorCode).withOpacity(double.tryParse(opacity) ?? 1.0);
...
//      assert(0.0 <= stop && stop <= 1.0);
  ...
  static Color _codeToColor(String code) {
    final X11Colors? webColor = WebColors.of(code);
    if (webColor != null) {
